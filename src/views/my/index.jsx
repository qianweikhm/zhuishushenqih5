...
import { getHelp, getHelpDetail } from '../../utils/api'
function Bookshelf() {
  ...
  const onLoadRefresh = async (isRefresh) => {
    const { helps } = await getHelp(infoObj)
    infoObj.start = list.length
    setList(v => {
      const newList = isRefresh ? helps : [...v, ...helps]
      return newList
    })
  }
  ...
  const getDetail = async (ele) => {
    const { post } = await getHelpDetail(ele._id)
    Dialog.confirm({
      title: post.title,
